...
createServer(async (request, response) => {
  try {
    ...
    let pathname = decodeURIComponent(url.pathname).replace(/^\/teje-con-medida\/?/, '');
...
}).listen(port, () => console.log(`TejeConMedida disponible en http://localhost:${port}/teje-con-medida/`));
